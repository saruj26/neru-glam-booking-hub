import { serviceCategoriesApi, galleryApi } from './apiService';
...
export async function getServiceCategoriesAsync(): Promise<ServiceCategory[]> {
  try {
    const cats = await serviceCategoriesApi.getAll();
    if (cats.length === 0) {
      await serviceCategoriesApi.bulkUpdate(DEFAULT_SERVICE_CATS);
      return DEFAULT_SERVICE_CATS;
    }
    return cats;
  } catch {
...
export async function saveServiceCategoriesAsync(cats: ServiceCategory[]): Promise<void> {
  await serviceCategoriesApi.bulkUpdate(cats);
}

export async function getGalleryImagesAsync(opts?: { featured?: boolean; limit?: number }): Promise<GalleryImage[]> {
  try {
    const images = await galleryApi.getImages({ active: true, ...opts });
    if (images.length === 0) {
      await galleryApi.bulkUpdateImages(DEFAULT_GALLERY_IMAGES);
      return DEFAULT_GALLERY_IMAGES.filter(i => i.active);
    }
    return images;
  } catch {
    return getGalleryImages().filter(i => i.active);
  }
}

export async function saveGalleryImagesAsync(imgs: GalleryImage[]): Promise<void> {
  await galleryApi.bulkUpdateImages(imgs);
}

export async function getBeforeAfterPairsAsync(): Promise<BeforeAfterPair[]> {
  try {
    const pairs = await galleryApi.getBeforeAfter();
    if (pairs.length === 0) {
      await galleryApi.bulkUpdateBeforeAfter(DEFAULT_BEFORE_AFTER);
      return DEFAULT_BEFORE_AFTER;
    }
    return pairs;
  } catch {
    return getBeforeAfterPairs();
  }
}

export async function saveBeforeAfterPairsAsync(pairs: BeforeAfterPair[]): Promise<void> {
  await galleryApi.bulkUpdateBeforeAfter(pairs);
}
...
export async function getGalleryCategoriesAsync(): Promise<GalleryCategory[]> {
  try {
    ...
    if (cats.length === 0) {
      await galleryApi.bulkUpdateCategories(DEFAULT_GALLERY_CATS);
      return DEFAULT_GALLERY_CATS;
    }
    return cats;
  } catch {
...
export async function saveGalleryCategoriesAsync(cats: GalleryCategory[]): Promise<void> {
  await galleryApi.bulkUpdateCategories(cats);
}

/* ═══ Synchronous localStorage fallbacks (backward compat) ═══════════════ */
...
export function saveServiceCategories(cats: ServiceCategory[]): void {
  write(K.svcCats, cats);
  saveServiceCategoriesAsync(cats).catch(() => {/* */});
}
...
export function saveGalleryCategories(cats: GalleryCategory[]): void {
  write(K.galCats, cats);
  saveGalleryCategoriesAsync(cats).catch(() => {/* */});
}
...
export function saveGalleryImages(imgs: GalleryImage[]): void {
  write(K.galImages, imgs);
  saveGalleryImagesAsync(imgs).catch(() => {/* */});
}
...
export function saveBeforeAfterPairs(pairs: BeforeAfterPair[]): void {
  write(K.beforeAfter, pairs);
  saveBeforeAfterPairsAsync(pairs).catch(() => {/* */});
}
